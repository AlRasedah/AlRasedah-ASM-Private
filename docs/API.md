...
| `/exposure-map` | External exposure map (`assets:read`): a bounded neighbourhood of observed relationships from `organization_id` or `asset_id` (`expand` for one node), with `depth` 1–4, `max_nodes` ≤ 300, `per_node` ≤ 100 and filters; every edge carries meaning, evidence, capability source, first/last seen and freshness. Not attack-path analysis. See [EXPOSURE_MAP.md](EXPOSURE_MAP.md) |
